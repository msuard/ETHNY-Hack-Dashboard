...
async generatePOKOfEphemeralKey(kBigInt, gBigInt, c1BigInt, pBigInt) {
    return new Promise(async (resolve, reject) => {
      try {

        const rBigInt = await this.Utils.getRandomNbitBigIntAsync(2048);
...
async generateDisjunctiveZKProofOfPlaintext(kBigInt, m, choicesList, gBigInt, ABigInt, c2BigInt, pBigInt) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let randomChallenges = await this.Utils.generateRandomNbitBigIntListWithRandomTailLengthAsync(2048, 10, validPlainTextsBigInt.length, 0, []);

        let randomResponses = await this.Utils.generateRandomNbitBigIntListWithRandomTailLengthAsync(4097, 20, validPlainTextsBigInt.length, 0, []); // RANDOM RESPONSES NEED TO BE UNDISTINGUISHABLE FROM REAL RESPONSE

        let rBigInt = await this.Utils.getRandomNbitBigIntAsync(2048);
        ...
        let eDBigInt = new BigInt(this.Sha3.sha3(stringToHash), 16).remainder(pBigInt); // CHALLENGE e // USE e mod p TO ALLOW FOR FASTER COMPUTATION ?
